...
def make_one_rpn_target(cfg, mode, input, window, truth_box, truth_label):
    ...
    label_assign = torch.from_numpy(label_assign).cuda()
    label_weight = torch.from_numpy(label_weight).cuda()
    target = torch.from_numpy(target).cuda()
    target_weight = torch.from_numpy(target_weight).cuda()
...
def make_rpn_target(cfg, mode, inputs, window, truth_boxes, truth_labels):
    ...
    for b in range(batch_size):
        ...
        # ignore paddings
        truth_box   = truth_box[truth_label != -1]
        truth_label = truth_label[truth_label != -1]
