...
class BboxReader(Dataset):
    ...
    def load_img(self, filename):
        ...
        img = img[np.newaxis,...]
        # clip the value of the image to 170
        img = np.clip(img,self.clip_min, self.clip_max)
        img = img.astype(np.float32)
        img = img.transpose(0, 3, 1, 2) # (1, d, h, w)
...
class Crop(object):
    ...
    def __call__(self, imgs, target, bboxes, isScale=False, isRand=False):
        ...
        # zz,yy,xx = np.meshgrid(np.linspace(normstart[0],normstart[0]+normsize[0],int(self.crop_size[0]/self.stride)),
