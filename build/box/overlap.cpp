...
int cpu_overlap(at::Tensor * boxes1, at::Tensor * boxes2, at::Tensor * overlap) {
    ...
    // Number of ROIs
    long long boxes1_num = boxes1->sizes()[0];
    long long boxes1_dim = boxes1->sizes()[1];

    long long boxes2_num = boxes2->sizes()[0];
    long long boxes2_dim = boxes2->sizes()[1];

    long long overlap_num = overlap->sizes()[0];
    long long overlap_dim = overlap->sizes()[1];
