...
int cpu_nms(at::Tensor * keep_out, at::Tensor * num_out, at::Tensor * boxes, at::Tensor * order, at::Tensor * areas, float nms_overlap_thresh) {
    ...
    // Number of ROIs
    long long boxes_num = boxes->sizes()[0];
    long long boxes_dim = boxes->sizes()[1];
    auto keep_out_flat = keep_out->data<long long>();
    auto boxes_flat = boxes->data<float>();
    auto order_flat = order->data<long long>();
    ...
    auto num_out_flat = num_out->data<long long>();
